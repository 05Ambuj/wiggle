...
const FollowingListModal = ({ isOpen, onClose, followings = [], onUnfollow }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-[#111] text-white w-full max-w-md max-h-[80vh] overflow-y-auto rounded-xl shadow-2xl border border-yellow-600/20 p-6 relative">
        ...
        <h2 className="text-xl font-bold text-yellow-400 mb-4">Following</h2>

        {followings.length === 0 ? (
          <p className="text-red-300">You're not following anyone yet.</p>
        ) : (
          <ul className="space-y-4">
            {followings.map((following) => {
              if (!following || typeof following !== "object") return null;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={following?.profilePic?.url || "/default-avatar.png"}
                      alt="avatar"
                  ...
                  <button
                    onClick={() => onUnfollow(following._id)}

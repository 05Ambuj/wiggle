...
export const getAllPost = TryCatch(async (req, res) => {
    const posts = await Post.find({ type: "post" }).sort({ createdAt: -1 }).populate("owner").populate({
      path:"comments.user",
      select:"-password"
    });
    const reels = await Post.find({ type: "reel" }).sort({ createdAt: -1 }).populate("owner").populate({
      path:"comments.user",
      select:"-password"
    });
